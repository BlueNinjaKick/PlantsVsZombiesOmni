...
void Plant::UpdateSweetPotato()
{
    ...
    else if (mState == PlantState::STATE_POTATO_ARMED)
    {
        ...
        if (FindTargetZombie(mRow, PlantWeapon::WEAPON_PRIMARY) && mPlantHealth < 1000)
        {
            mSweetCountdown--;
            if (mSweetCountdown <= 10) {
                DoSpecial();
            }
        }
        else
...
void Plant::UpdateAbilities()
{
    ...
    else if (mSeedType == SeedType::SEED_SWEET_POTATO)                                          UpdateSweetPotato();
...
void Plant::DoBlink()
{
    ...
    if (mSeedType == SeedType::SEED_POTATOMINE && mState != PlantState::STATE_POTATO_ARMED)
        return;
    if (mSeedType == SeedType::SEED_SWEET_POTATO && mState != PlantState::STATE_POTATO_ARMED)
        return;
...
Rect Plant::GetPlantAttackRect(PlantWeapon thePlantWeapon)
{
    ...
    case SeedType::SEED_POTATOMINE||SEED_SWEET_POTATO:     aRect = Rect(mX,            mY,             mWidth - 25,        mHeight);               break;
